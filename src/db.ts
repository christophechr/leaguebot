...
export async function ensureHansFlashTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS hans_flash_counter (
      split TEXT NOT NULL,
      year INTEGER NOT NULL,
      team TEXT NOT NULL DEFAULT '',
      count INTEGER NOT NULL,
  ...
  await pool.query(`
    ALTER TABLE hans_flash_counter
    ALTER COLUMN team SET DEFAULT ''
  `);
  await pool.query(`UPDATE hans_flash_counter SET team = '' WHERE team IS NULL`);
  await pool.query(`
    ALTER TABLE hans_flash_counter
    ALTER COLUMN team SET NOT NULL
  `);
...
export async function getHansFlashCount(split: string, year: number, team?: string | null) {
  if (team) {
    const result = await pool.query(
      `
        SELECT count
        FROM hans_flash_counter
        WHERE split = $1 AND year = $2 AND team = $3
      `,
      [split, year, team]
    );
...
export async function incrementHansFlashCount(
  split: string,
  year: number,
  team?: string | null,
  amount = 1
  ...
  const result = await pool.query(
    `
      INSERT INTO hans_flash_counter (split, year, team, count)
      VALUES ($1, $2, $3, $4)
      ON CONFLICT (split, year, team)
      DO UPDATE SET count = hans_flash_counter.count + EXCLUDED.count
      RETURNING count
    `,
    [split, year, team ?? "", amount]
  );
...
export async function getHansFlashCountForYear(year: number, team?: string | null) {
  if (team) {
    const result = await pool.query(
      `
        SELECT COALESCE(SUM(count), 0) AS count
